...
class BoxlessDataRow {
  ...
  /// table.
  final List<BoxlessDataCell> cells;

  bool get _debugInteractive => onSelectChanged != null || cells.any((BoxlessDataCell cell) => cell._debugInteractive);
}
...
class BoxlessDataCell {
  ...
  static const BoxlessDataCell empty = BoxlessDataCell(SizedBox(width: 0.0, height: 0.0));
...
class BoxlessDataTable extends StatelessWidget {
  ...
  BoxlessDataTable({
    ...
    @required this.rows,
        ...
        assert(rows != null),
        assert(!rows.any((BoxlessDataRow row) => row.cells.length != columns.length)),
        assert(dividerThickness != null && dividerThickness >= 0),
        _onlyTextColumn = _initOnlyTextColumn(columns),
        super(key: key);

  /// The configuration and labels for the columns in the table.
  final List<BoxlessDataColumn> columns;
  ...
  final List<BoxlessDataRow> rows;

  // Set by the constructor to the index of the only Column that is
  // non-numeric, if there is exactly one, otherwise null.
  final int _onlyTextColumn;
  static int _initOnlyTextColumn(List<BoxlessDataColumn> columns) {
    int result;
    for (int index = 0; index < columns.length; index += 1) {
      final BoxlessDataColumn column = columns[index];
      if (!column.numeric) {
        if (result != null)
          return null;
        result = index;
  ...
  bool get _debugInteractive {
    return columns.any((BoxlessDataColumn column) => column._debugInteractive)
        || rows.any((BoxlessDataRow row) => row._debugInteractive);
  }
  ...
  void _handleSelectAll(bool checked) {
    if (onSelectAll != null) {
      ...
      for (final BoxlessDataRow row in rows) {
  ...
  Widget build(BuildContext context) {
    ...
    for (int dataColumnIndex = 0; dataColumnIndex < columns.length; dataColumnIndex += 1) {
      final BoxlessDataColumn column = columns[dataColumnIndex];
      ...
      rowIndex = 1;
      for (final BoxlessDataRow row in rows) {
        final BoxlessDataCell cell = row.cells[dataColumnIndex];
...
class BoxlessTableRowInkWell extends InkResponse {
  /// Creates an ink well for a table row.
  const BoxlessTableRowInkWell({
    Key key,
    Widget child,
...
class _BoxlessSortArrow extends StatefulWidget {
  const _BoxlessSortArrow({
    Key key,
    this.visible,
    this.down,
    this.duration,
  ...
  @override
  _BoxlessSortArrowState createState() => _BoxlessSortArrowState();
}

class _BoxlessSortArrowState extends State<_BoxlessSortArrow> with TickerProviderStateMixin {
  ...
  @override
  void didUpdateWidget(_BoxlessSortArrow oldWidget) {
